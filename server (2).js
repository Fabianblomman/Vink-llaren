...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST' && path_ === '/analyze') {
      ...
      const { imageBase64, mediaType, backBase64, backMediaType, prompt } = body;
      const currentYear = new Date().getFullYear();

      const defaultPrompt = `Du är en expert på vin och vinlagring. Analysera etiketten på vinflaskan i bilden och svara ENBART med ett JSON-objekt utan markdown eller förklaringar.
...
Svara ENBART med JSON.`;
      ...
      // Bygg innehållsarrayen — lägg till baksida om den finns
      const content = [
      ...
      content.push({ type: 'text', text: prompt || defaultPrompt });

      const aiRes = await anthropicRequest([{ role: 'user', content }]);
